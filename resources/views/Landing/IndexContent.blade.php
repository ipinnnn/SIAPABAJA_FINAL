...
*/
if (!function_exists('buildDokumenListForLanding')) {
    function buildDokumenListForLanding($pengadaan){
        if(!$pengadaan) return [];

        $attrs = method_exists($pengadaan, 'getAttributes')
            ? $pengadaan->getAttributes()
            : (array)$pengadaan;
        ...
        foreach($attrs as $field => $rawValue){
            ...
            $files = array_values(array_filter(array_map(function($x){
                ...
                if(Str::startsWith($s, 'public/')) {
                    $s = Str::after($s, 'public/');
                }

                if(Str::startsWith($s, 'storage/')) {
                    $s = Str::after($s, 'storage/');
                }
...
if (!function_exists('buildDocNoteForLanding')) {
    function buildDocNoteForLanding($pengadaan){
        ...
        if(is_array($rawE) && count($rawE) > 0){
            return implode(', ', array_map(
                fn($x) => is_string($x) ? $x : json_encode($x),
                $rawE
            ));
        }
...
================================ */
  ...
  /**
   * Fetch data chart realtime dari server.
   * Paramnya sama persis dengan filter di pbj.blade.php:
   *   - tahun    → ?tahun=2024
   *   - unit_id  → ?unit_id=3
   */
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    /* ---- Update chart dari API (opsi filter sudah dirender partial, tidak di-overwrite) ---- */
    async function updateDonut() {
      if (!donutChart) return;
      try {
        const data = await fetchChartStats(donutYearEl?.value || '', donutUnitEl?.value || '');
    ...
    async function updateBar() {
      ...
      try {
        ...
        barChart.data.labels = data.metode.labels.map(label => {
          return label.split(' ');
        });
    ...
    /* ---- Load data awal ---- */
    await Promise.all([updateDonut(), updateBar()]);
  });
</script>
@endpush